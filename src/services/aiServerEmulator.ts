...
const CONFIGURED_GEMINI_MODEL = import.meta.env.VITE_GEMINI_MODEL?.trim() || null;
...
async function callGemini(prompt: string): Promise<unknown> {
    ...
    throw new ApiError(
        ...
        "Nu am gasit un model Gemini disponibil.",
...
function buildLearningPathMock(input: LearningPathRequest): LearningPathResponse {
    ...
    return {
        ...
        levels,
        notes: [
            "Mock local: configureaza VITE_GEMINI_API_KEY pentru raspunsuri AI reale.",
...
function buildExerciseMock(input: GenerateExercisesRequest): GenerateExercisesResponse {
    ...
    return {
        exerciseSet: {
            id: `ai-set-${globalThis.crypto.randomUUID()}`,
...
function adjustExerciseCount(
    ...
    return {
        exerciseSet: {
            ...
            id: response.exerciseSet.id || `ai-set-${globalThis.crypto.randomUUID()}`,
