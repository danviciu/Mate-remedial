...
import { loadStudentsState } from "../utils/studentsStore.js";
...
const randomizeQuestionOptions = (question, avoidCorrectIndex = null) => {
  ...
  const shuffledChoices = shuffle(indexed);
  let nextCorrectIndex = shuffledChoices.findIndex((item) => item.correct);
  const hasAvoidTarget =
    Number.isInteger(avoidCorrectIndex) &&
    avoidCorrectIndex >= 0 &&
    avoidCorrectIndex < shuffledChoices.length;

  if (hasAvoidTarget && nextCorrectIndex === avoidCorrectIndex && shuffledChoices.length > 1) {
    const swapIndex = shuffledChoices.findIndex((_, idx) => idx !== avoidCorrectIndex);
    if (swapIndex >= 0) {
      [shuffledChoices[nextCorrectIndex], shuffledChoices[swapIndex]] = [
        shuffledChoices[swapIndex],
        shuffledChoices[nextCorrectIndex],
      ];
...
const getAvoidCorrectIndex = (queue) => {
  if (!Array.isArray(queue) || queue.length < 2) return null;
  const last = queue[queue.length - 1]?.correctIndex;
  const beforeLast = queue[queue.length - 2]?.correctIndex;
  if (!Number.isInteger(last) || !Number.isInteger(beforeLast)) return null;
  return last === beforeLast ? last : null;
};
...
const buildQuestionQueue = ({ moduleId, difficulty, roundsTotal, teamCount, spiceMode }) => {
  ...
  while (queue.length < totalTurns) {
    ...
    if (!prev || prev.id !== source.id || ordered.length === 1) {
      const avoidCorrectIndex = getAvoidCorrectIndex(queue);
      queue.push({
        ...randomizeQuestionOptions(source, avoidCorrectIndex),
        gameQuestionId: `${source.id}-${queue.length}`,
      });
    }
    cursor += 1;
  ...
  while (queue.length < totalTurns) {
    ...
    queue.push({
      ...randomizeQuestionOptions(source, avoidCorrectIndex),
      gameQuestionId: `${source.id}-repeat-${queue.length}`,
    });
  }
  return queue;
};
...
const renderQuestionVisual = (question) => {
  ...
  if (question.visualType === "fractionBar") {
    return (
      <FractionVisual
        totalParts={data.totalParts}
        takenParts={data.takenParts}
        hideSummary
      />
    );
  }
...
const normalizeStudentSelections = (ids, targetTeamCount, students, fillMissing) => {
  const safeIds = Array.isArray(ids) ? ids : [];
  const safeStudents = Array.isArray(students) ? students : [];
  const availableIds = new Set(safeStudents.map((student) => student.id));
  const next = [];
  for (let idx = 0; idx < targetTeamCount; idx += 1) {
    const current = safeIds[idx] ?? "";
    if (current && availableIds.has(current) && !next.includes(current)) {
      next.push(current);
      continue;
    }
    if (fillMissing) {
      const fallback = safeStudents.find((student) => !next.includes(student.id));
...
export default function TeamQuiz({ goHome }) {
  ...
  const superSimpleMode = appSettings.superSimpleMode === true;
  ...
  const [studentsState, setStudentsState] = useState(() => loadStudentsState());
  const [useEnrolledStudents, setUseEnrolledStudents] = useState(false);
  const [selectedStudentIds, setSelectedStudentIds] = useState(() => Array.from({ length: 4 }, () => ""));
  ...
  const effectiveSettings = useMemo(() => {
    if (!superSimpleMode) return settings;
    return {
      ...settings,
      timerSeconds: 0,
      powerUps: false,
  ...
  }, [settings, superSimpleMode]);

  const allowAnimations = effectiveSettings.animations && !reducedMotion;
  const timerEnabled = effectiveSettings.timerSeconds > 0;
  ...
  const enrolledStudents = studentsState.students;
  const studentsById = useMemo(
    () => new Map(enrolledStudents.map((student) => [student.id, student])),
    [enrolledStudents],
  );
  const studentsByGrade = useMemo(() => {
    const groups = new Map();
    enrolledStudents.forEach((student) => {
      const grade = typeof student.grade === "string" && student.grade.trim().length > 0
        ? student.grade.trim()
        : "Nespecificata";
    ...
    return Array.from(groups.entries())
      .sort(([left], [right]) => left.localeCompare(right, "ro", { numeric: true, sensitivity: "base" }))
      .map(([grade, items]) => ({
        grade,
        items: [...items].sort((a, b) => a.name.localeCompare(b.name, "ro", { sensitivity: "base" })),
      }));
  }, [enrolledStudents]);
  ...
  const setTurn = (teamIdx, turnRound, pointer, teamsSnapshot = teams) => {
    ...
    setRoundNotice(`Randul: ${teamsSnapshot[teamIdx]?.name ?? `Echipa ${teamIdx + 1}`}`);
    setTimeLeft(effectiveSettings.timerSeconds);
  ...
  const startSetup = () => {
    ...
    setSetupError("");
    setSetupTab("game");
    const latestStudentsState = loadStudentsState();
    setStudentsState(latestStudentsState);
    setSelectedStudentIds((prev) =>
      normalizeStudentSelections(prev, teamCount, latestStudentsState.students, useEnrolledStudents),
    );
  ...
  const startGame = () => {
    const latestStudentsState = loadStudentsState();
    const latestStudents = Array.isArray(latestStudentsState.students) ? latestStudentsState.students : [];
    setStudentsState(latestStudentsState);
    if (useEnrolledStudents && latestStudents.length < teamCount) {
      setSetupError(`Ai nevoie de cel putin ${teamCount} elevi inscrisi pentru acest battle.`);
      setSetupTab("students");
      return;
    }
    const queue = buildQuestionQueue({ moduleId, difficulty, roundsTotal, teamCount, spiceMode: effectiveSettings.spiceMode });
    if (!queue.length) {
      setSetupError("Nu exista intrebari pentru setarile alese.");
      return;
    }
    setSelectedStudentIds((prev) => normalizeStudentSelections(prev, teamCount, latestStudents, useEnrolledStudents));
    setTeams(createTeams(teamCount, effectiveSettings.powerUps));
  ...
  const updateTeamName = (teamIndex, value) => {
    setTeams((prev) =>
      prev.map((team, idx) => (
        idx === teamIndex
          ? { ...team, name: value.trim().length > 0 ? value.slice(0, 24) : `Echipa ${team.id}` }
          : team
      )),
    );
  };
  ...
  const startFirstRound = () => {
    let nextTeams = teams;
    if (useEnrolledStudents) {
      if (enrolledStudents.length < teams.length) {
        setSetupError(`Ai nevoie de cel putin ${teams.length} elevi inscrisi pentru battle.`);
        return;
      }
      const selectedIds = selectedStudentIds.slice(0, teams.length);
      if (selectedIds.some((id) => !id)) {
        setSetupError("Selecteaza cate un elev pentru fiecare echipa.");
        return;
      }
      if (new Set(selectedIds).size !== selectedIds.length) {
        setSetupError("Acelasi elev nu poate fi ales in doua echipe.");
        return;
      }
      const selectedStudents = selectedIds.map((id) => studentsById.get(id));
      if (selectedStudents.some((student) => !student)) {
        setSetupError("Selectia de elevi este invalida. Realege echipele.");
        return;
      }
      nextTeams = teams.map((team, idx) => ({
        ...team,
        name: selectedStudents[idx].name,
        studentId: selectedStudents[idx].id,
        studentGrade: selectedStudents[idx].grade,
      }));
    } else {
      nextTeams = teams.map((team) => ({
        ...team,
        name: team.name.trim().length > 0 ? team.name.trim().slice(0, 24) : `Echipa ${team.id}`,
        studentId: null,
        studentGrade: null,
      }));
    }
  ...
  const finishAttempt = ({ correct, targetTeamIndex, pointsAwarded, reason }) => {
    ...
    clearAutoAdvance();
    autoAdvanceRef.current = window.setTimeout(goNextTurn, effectiveSettings.autoAdvanceDelayMs);
  };
  ...
  const onSelectAnswer = (optionIndex) => {
    ...
    if (!correct && effectiveSettings.stealMode === "on" && !stealActive) {
      enterSteal("Gresit.");
      return;
    }
  ...
  const onTimeExpired = () => {
    ...
    if (effectiveSettings.stealMode === "on" && !stealActive) {
  ...
  const applyPowerUp = (type, teamIndex) => {
    if (!effectiveSettings.powerUps || phase !== PHASES.question || lockAnswers || !question || teamIndex !== respondingTeamIndex) return;
  ...
  }, [phase, moduleId, difficulty, roundsTotal, effectiveSettings, teams]);
      ...
      <div className="mx-auto max-w-7xl">
          ...
          <section className="rounded-3xl border border-indigo-100 bg-white/90 p-6 shadow-lg">
            ...
            {superSimpleMode ? (
              <div className="mt-4 rounded-2xl border border-cyan-200 bg-cyan-50 px-4 py-3">
                <p className="text-sm font-black text-cyan-800">Mod super simplu activ</p>
                <p className="text-sm text-cyan-700">
                  In joc: timer OFF, steal OFF, power-ups OFF, enunt mai mare si ghid pas cu pas.
                </p>
              </div>
            ) : null}

            <div className="mt-5 inline-flex rounded-2xl border border-indigo-200 bg-indigo-50 p-1">
              <button
                type="button"
                onClick={() => setSetupTab("game")}
                className={`rounded-xl px-4 py-2 text-sm font-black transition ${setupTab === "game" ? "bg-indigo-600 text-white shadow" : "text-indigo-700 hover:bg-indigo-100"}`}
              >
                Joc
              </button>
              <button
                ...
                className={`rounded-xl px-4 py-2 text-sm font-black transition ${setupTab === "students" ? "bg-indigo-600 text-white shadow" : "text-indigo-700 hover:bg-indigo-100"}`}
              >
                Elevi pe clase ({enrolledStudents.length})
              </button>
            </div>
            ...
            {setupTab === "game" ? (
              <>
                <div className="mt-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                  ...
                  <label className="text-sm font-bold text-indigo-700">Dificultate<select value={difficulty} onChange={(e) => setDifficulty(e.target.value)} disabled={effectiveSettings.spiceMode === "mixed"} className="mt-2 w-full rounded-2xl border-2 border-indigo-200 px-4 py-3"><option value="easy">Usor</option><option value="medium">Mediu</option><option value="review">Recapitulare</option></select></label>
                  <label className="text-sm font-bold text-indigo-700">Echipe<select value={teamCount} onChange={(e) => {
                    const nextTeamCount = Number(e.target.value);
                    setTeamCount(nextTeamCount);
                    setSelectedStudentIds((prev) =>
                      normalizeStudentSelections(prev, nextTeamCount, enrolledStudents, useEnrolledStudents),
                    );
                  }} className="mt-2 w-full rounded-2xl border-2 border-indigo-200 px-4 py-3">{[2, 3, 4].map((n) => <option key={n} value={n}>{n}</option>)}</select></label>
                  ...
                  <div className="mt-3 grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                    <label className="text-sm font-semibold text-indigo-700">
                      ...
                      <select
                        ...
                        onChange={(e) => setSettings((p) => ({ ...p, timerSeconds: Number(e.target.value) }))}
                        disabled={superSimpleMode}
                        className="mt-1 w-full rounded-xl border border-indigo-200 px-3 py-2 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                        {TIMER_OPTIONS.map((v) => (
                    ...
                    <label className="text-sm font-semibold text-indigo-700">
                      ...
                      <select
                        ...
                        onChange={(e) => setSettings((p) => ({ ...p, autoAdvanceDelayMs: Number(e.target.value) }))}
                        className="mt-1 w-full rounded-xl border border-indigo-200 px-3 py-2"
                      >
                        {AUTO_DELAY_OPTIONS.map((v) => (
                          <option key={v} value={v}>
                            {v} ms
                          </option>
                        ))}
                    ...
                    <label className="text-sm font-semibold text-indigo-700">
                      Steal mode
                      <select
                        value={settings.stealMode}
                        onChange={(e) =>
                          setSettings((p) => ({ ...p, stealMode: e.target.value === "on" ? "on" : "off" }))
                        }
                        disabled={superSimpleMode}
                        className="mt-1 w-full rounded-xl border border-indigo-200 px-3 py-2 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                    ...
                    <label className="text-sm font-semibold text-indigo-700">
                      Difficulty spice
                      <select
                        value={settings.spiceMode}
                        onChange={(e) =>
                          setSettings((p) => ({ ...p, spiceMode: e.target.value === "mixed" ? "mixed" : "normal" }))
                        }
                        disabled={superSimpleMode}
                        className="mt-1 w-full rounded-xl border border-indigo-200 px-3 py-2 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                    ...
                    <label className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-white px-3 py-2 text-sm font-semibold text-indigo-700">
                      <input
                        type="checkbox"
                        checked={settings.sounds}
                        onChange={(e) => setSettings((p) => ({ ...p, sounds: e.target.checked }))}
                        className="h-4 w-4"
                      />
                      Sounds
                    </label>
                    <label className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-white px-3 py-2 text-sm font-semibold text-indigo-700">
                      <input
                        ...
                        onChange={(e) => setSettings((p) => ({ ...p, animations: e.target.checked }))}
                        className="h-4 w-4"
                      />
                      Animations
                    </label>
                    <label className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-white px-3 py-2 text-sm font-semibold text-indigo-700">
                      <input
                        ...
                        onChange={(e) => setSettings((p) => ({ ...p, powerUps: e.target.checked }))}
                        disabled={superSimpleMode}
                        className="h-4 w-4"
                      />
                      Power-ups
              ...
              <div className="mt-6 rounded-2xl border border-indigo-200 bg-indigo-50 p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm font-black uppercase tracking-wide text-indigo-700">Catalog elevi pentru Team Quiz</p>
                  <TqButton
                    type="button"
                    onClick={() => {
                      const latestStudentsState = loadStudentsState();
                    ...
                    className="rounded-xl border border-indigo-200 bg-white px-3 py-2 text-sm font-bold text-indigo-700 hover:bg-indigo-100"
                  >
                    Refresh elevi
                  </TqButton>
                </div>
                {studentsByGrade.length ? (
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    {studentsByGrade.map((group) => (
                      <div key={`grade-${group.grade}`} className="rounded-xl border border-indigo-200 bg-white p-3">
                        <p className="text-sm font-black text-indigo-700">Clasa {group.grade}</p>
                        <ul className="mt-2 space-y-1 text-sm text-indigo-900">
                          {group.items.map((student) => (
                            <li key={student.id} className="rounded-lg bg-indigo-50 px-2 py-1 font-semibold">
                              {student.name}
                            </li>
                          ))}
                        </ul>
                  ...
                  <p className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">
                    Nu exista elevi inscrisi. Adauga elevi in tabul Profesor si revino aici.
                  </p>
                )}
              </div>
            )}
        ...
        {phase === PHASES.teamSetup ? (
          <section className="rounded-3xl border border-indigo-100 bg-white/90 p-6 shadow-lg">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-2 rounded-2xl border border-indigo-200 bg-indigo-50 px-4 py-3">
              <label className="inline-flex items-center gap-2 text-sm font-bold text-indigo-700">
                <input
                  type="checkbox"
                  checked={useEnrolledStudents}
                  onChange={(e) => {
                    const checked = e.target.checked;
                    const latestStudentsState = loadStudentsState();
                    setStudentsState(latestStudentsState);
                    setUseEnrolledStudents(checked);
                    setSelectedStudentIds((prev) =>
                      normalizeStudentSelections(prev, teamCount, latestStudentsState.students, checked),
                    );
                  }}
                  disabled={!enrolledStudents.length}
                  className="h-4 w-4"
                />
                Selecteaza elevii inscrisi pentru battle
              </label>
              <p className="text-xs font-semibold text-indigo-700">Elevi disponibili: {enrolledStudents.length}</p>
            </div>
            {useEnrolledStudents && !enrolledStudents.length ? (
              <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-800">
                Nu exista elevi inregistrati. Revino la tabul Elevi pe clase si adauga elevi.
              </p>
            ) : null}

            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {teams.map((team, index) => {
                const selectedStudent = studentsById.get(selectedStudentIds[index] ?? "");
                const usedByOtherTeams = new Set(selectedStudentIds.filter((id, idx) => idx !== index && id));
                return (
                  <label key={team.id} className="rounded-2xl border-2 border-indigo-200 bg-indigo-50 p-4">
                    <span className="text-xs font-black uppercase tracking-wide text-indigo-500">Echipa {team.id}</span>
                    {useEnrolledStudents ? (
                      <>
                        <select
                          ...
                          className="mt-2 w-full rounded-xl border border-indigo-200 bg-white px-3 py-2 text-lg font-bold text-indigo-800"
                        >
                          <option value="">Alege elev</option>
                          {studentsByGrade.map((group) => (
                            <optgroup key={`team-${team.id}-${group.grade}`} label={`Clasa ${group.grade}`}>
                              {group.items.map((student) => (
                                <option key={student.id} value={student.id} disabled={usedByOtherTeams.has(student.id)}>
                                  {student.name}
                                </option>
                              ))}
                            </optgroup>
                        ...
                        <p className="mt-2 text-xs font-semibold text-indigo-700">
                          {selectedStudent ? `Clasa ${selectedStudent.grade}` : "Selecteaza elevul pentru aceasta echipa."}
                        </p>
                      </>
                    ) : (
                      <input
            ...
            {setupError ? <p className="mt-4 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{setupError}</p> : null}
            <TqButton type="button" onClick={startFirstRound} className="mt-6 rounded-2xl bg-emerald-600 px-6 py-4 text-lg font-black text-white shadow hover:bg-emerald-700">Start runda 1</TqButton>
          </section>
        ) : null}
        ...
        {(phase === PHASES.question || phase === PHASES.feedback) && question ? (
          <section className="relative overflow-hidden rounded-3xl border border-indigo-100 bg-white/95 p-4 shadow-lg sm:p-6">
            <div className="mb-4 flex flex-col items-center justify-between gap-3 rounded-2xl border border-indigo-200 bg-indigo-50 p-4 lg:flex-row">
              ...
              {timerEnabled ? <CountdownRing totalSeconds={effectiveSettings.timerSeconds} remainingSeconds={timeLeft} paused={timerPaused || freezeSeconds > 0 || phase === PHASES.feedback} motionEnabled={allowAnimations} /> : <div className="rounded-full bg-indigo-100 px-5 py-2 text-sm font-bold text-indigo-700">Fara timer</div>}
            </div>
            ...
            <h2 className={`${superSimpleMode ? "text-4xl sm:text-5xl" : "text-3xl sm:text-4xl"} text-center font-black leading-tight text-indigo-900`}>
              {question.text}
            </h2>
            {superSimpleMode ? (
              <div className="mx-auto mt-4 max-w-4xl rounded-2xl border border-amber-200 bg-amber-50 p-4">
                <p className="text-sm font-black uppercase tracking-wide text-amber-700">Ghid rapid</p>
                <ol className="mt-2 list-decimal space-y-1 pl-5 text-base font-semibold text-amber-900">
                  <li>Citeste intreaga intrebare.</li>
                  <li>Uita-te la vizual si extrage numerele importante.</li>
                  <li>Elimina variantele imposibile, apoi alege raspunsul.</li>
                </ol>
                <div className="mt-3 flex flex-wrap gap-2">
                  <TqButton
                    type="button"
                    onClick={() => setShowSimpleGuide((prev) => !prev)}
                    className="rounded-xl bg-white px-3 py-2 text-sm font-bold text-amber-800 shadow-sm hover:bg-amber-100"
                  >
                    {showSimpleGuide ? "Ascunde explicatia" : "Arata explicatia scurta"}
                  </TqButton>
                </div>
                {showSimpleGuide ? (
                  <p className="mt-3 rounded-xl bg-white px-3 py-2 text-sm font-semibold text-amber-900">
                    {question.explanation || "Verifica varianta corecta comparand cu enuntul si cu vizualul."}
                  </p>
                ) : null}
              </div>
            ) : null}
            ...
            <div className="mx-auto mt-6 grid max-w-5xl gap-3 md:grid-cols-3">
              {question.choices.map((choice, index) => {
                ...
                let cls = "w-full rounded-2xl border-2 px-4 py-4 text-left font-black transition";
                cls += superSimpleMode ? " min-h-20 text-2xl" : " min-h-16 text-xl";
