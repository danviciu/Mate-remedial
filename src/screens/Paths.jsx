...
export default function Paths({
  ...
  const initialModule = useMemo(
    () => MODULES.find((item) => item.id === initialModuleId) ?? null,
    [initialModuleId],
  );
  const initialSelectedLevel = useMemo(() => {
    if (!autoStart || !initialModule) return null;
    const requestedLevelId = normalizeLevelId(initialLevelId);
    return (
      initialModule.levels.find(
        (level) => normalizeLevelId(level.id ?? level.level) === requestedLevelId,
      ) ?? initialModule.levels[0] ?? null
    );
  }, [autoStart, initialLevelId, initialModule]);

  const [selectedModuleId, setSelectedModuleId] = useState(() => initialModule?.id ?? null);
