...
export default function Player({ module, level, onBack, onOpenLesson, onOpenSimulator }) {
  ...
  const handleHint = () => {
    const hintsCount = activeStep?.hints?.length ?? 0;
    ...
    <div className="bg-white/80 backdrop-blur rounded-2xl shadow-lg p-6 transition-all duration-300">
      ...
      {!activeComplete && activeExercise && activeStep ? (
        <>
          ...
          <p className="text-gray-700 mb-3 text-lg">{activeExercise.prompt}</p>
          <p className="text-xl font-semibold text-indigo-700 mb-4">{activeStep.prompt}</p>

          <div className="space-y-3">
            {activeStep.choices.map((choice, index) => {
              const isSelected = selectedChoice === index;
              let choiceClass =
                "w-full text-left p-4 rounded-xl border-2 bg-white hover:bg-indigo-50 border-indigo-100 transition-all duration-200 text-lg active:scale-[0.99]";
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <button
              type="button"
              onClick={handleHint}
              className="px-4 py-2 rounded-xl bg-amber-100 text-amber-900 hover:bg-amber-200 transition"
            >
              Ajutor
            </button>
            <button
          ...
          {visibleHintLevel > 0 ? (
            <div className="mt-4 rounded-xl bg-amber-50 border border-amber-200 p-4">
              <p className="font-semibold text-amber-900 mb-1">Indiciu {visibleHintLevel}</p>
              <p className="text-amber-900">{activeStep.hints[visibleHintLevel - 1]}</p>
            </div>
          ) : null}
