...
function FractionOperationVisual({ spec }) {
...
function renderVisual(spec) {
  ...
  switch (spec.type) {
    ...
    case "fractionOperation":
      return <FractionOperationVisual spec={spec} />;
    case "percentGrid":
      return <PercentGrid percent={spec.percent} label={spec.label} />;
    case "percentBar":
...
export default function VisualRenderer({
  ...
  const visual = useMemo(() => renderVisual(spec), [spec]);
