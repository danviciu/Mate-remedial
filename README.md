...
`npm run dev` porneste acum atat API-ul AI, cat si clientul Vite.
...
## Modul AI (proxy + server local)

Rulare in doua terminale:

1. API server (port 8787 implicit):
```bash
npm run server
```

2. Client Vite (fara API):
```bash
npm run dev:web
```

Sau intr-o singura comanda (api + client):

```bash
npm run dev
```

Configurare cheie in `mate-reset/.env.local`:
...
Frontend-ul apeleaza doar endpointuri locale `/api/...` prin proxy Vite:
- `POST /api/ai/learning-path`
- `POST /api/ai/generate-exercises`
- `POST /api/ai/solve-step-by-step`
